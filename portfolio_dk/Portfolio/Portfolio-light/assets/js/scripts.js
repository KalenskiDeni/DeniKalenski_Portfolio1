    ...
    $(document).ready(function () {
        ...
        function updateNavbarStyles() {
            var bodyScroll = $(window).scrollTop(),
                ...
                navLinks = $(".navbar .navbar-nav .nav-link");
        
            if (bodyScroll > 300) { // Change value to adjust scroll position
                navbar.addClass("nav-scroll");  // Add class for scroll styles
                logo.attr('src', 'assets/imgs/LOGO-DARK.png');  // Switch to dark logo on scroll
                navLinks.css('color', '#1a1a1a');  // Dark text for nav links when scrolled
            } else {
                navbar.removeClass("nav-scroll");  // Remove scroll-specific class
                setNavLinkColors();  // Set logo and nav link colors based on the page
            }
        }
    
        // Function to set nav link colors and logo based on the page
        function setNavLinkColors() {
            var navLinks = $(".navbar .navbar-nav .nav-link"),
                logo = $(".navbar .logo > img");
    
            // Default: white text and white logo unless on specific pages
            logo.attr('src', 'assets/imgs/logo-white.png');
            navLinks.css('color', '#ffffff');
    
            // Apply dark text and dark logo for specific pages
            if ($('body').hasClass('photography-page') ||
                $('body').hasClass('page-about') ||
                $('body').hasClass('page-contact') ||
                $('body').hasClass('page-portfolio')) {
                logo.attr('src', 'assets/imgs/LOGO-DARK.png'); // Use dark logo on these pages
                navLinks.css('color', '#1a1a1a'); // Dark text on specific pages
            }
        }
...
let currentSlide123 = 1; // Start from the second slide
let isGalleryView123 = false; // Define and initialize the variable

function initSlider123() {
    const slidesContainer = document.querySelector('.slides-123');
    const slides = document.querySelectorAll('.slide-123');
    ...
    // Append clones to the beginning and end
    slidesContainer.appendChild(firstSlideClone);
    slidesContainer.insertBefore(lastSlideClone, slidesContainer.firstChild);

    // Set the initial position
    slidesContainer.style.transform = `translateX(-${100}%)`;
    slidesContainer.style.transition = 'none'; // Disable transition for initial setup

    // Add transition effect back after setup
    setTimeout(() => {
...
function showSlide123(index) {
    ...
    // Adjust index for infinite loop
    if (index > totalSlides) {
        slidesContainer.style.transition = 'none'; // Disable transition for reset
        slidesContainer.style.transform = `translateX(-${(index - totalSlides) * 100}%)`;
        currentSlide123 = 1;
        setTimeout(() => {
            slidesContainer.style.transition = 'transform 0.5s ease';
            slidesContainer.style.transform = `translateX(-${currentSlide123 * 100}%)`;
        }, 50);
    } else if (index < 1) {
        slidesContainer.style.transition = 'none'; // Disable transition for reset
        slidesContainer.style.transform = `translateX(-${(totalSlides) * 100}%)`;
        currentSlide123 = totalSlides;
        setTimeout(() => {
            slidesContainer.style.transition = 'transform 0.5s ease';
            slidesContainer.style.transform = `translateX(-${currentSlide123 * 100}%)`;
        }, 50);
    } else {
        currentSlide123 = index;
        slidesContainer.style.transform = `translateX(-${currentSlide123 * 100}%)`;
    }
}

function nextSlide123() {
    showSlide123(currentSlide123 + 1);
}

function prevSlide123() {
    showSlide123(currentSlide123 - 1);
}

// Optional: Automatically switch slides every 5 seconds
setInterval(() => {
    if (!isGalleryView123) {
        nextSlide123();
    }
...
// Toggle between slider and gallery view
function toggleGalleryView123() {
    const slider = document.querySelector('.slider-123');
    const gallery = document.querySelector('.gallery-123');
    const toggleButton = document.getElementById('toggleButton123');
    ...
    if (isGalleryView123) {
        ...
        toggleButton.textContent = 'Slider View';
    }

    isGalleryView123 = !isGalleryView123;
}

// Initialize the slider by showing the first slide
initSlider123();
showSlide123(currentSlide123);
